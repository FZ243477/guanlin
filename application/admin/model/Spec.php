...
class Spec extends Model {
   ...
   /**
     ...
     * @param int $id 规格id
     */
    public function afterSave($id, $post_items)
    {
        
        $model = model("SpecItem"); // 实例化User对象
        ...
        $db_items = $model->where("spec_id = $id")->field('id,item')->select();
        $db_items = get_id_val($db_items, 'id','item');
        // 两边 比较两次
        $dataList = [];
...
//                $res = model("SpecGoodsPrice")->where("`key` REGEXP '^{$key}_' OR `key` REGEXP '_{$key}_' OR `key` REGEXP '_{$key}$' or `key` = '{$key}'")->select(); // 删除规格项价格表
                model("SpecGoodsPrice")->where("`key` REGEXP '^{$key}_' OR `key` REGEXP '_{$key}_' OR `key` REGEXP '_{$key}$' or `key` = '{$key}'")->delete();
                model("SpecItem")->destroy($key); // 删除规格项
            }
        }        
    }    
}
