...
trait GoodsHelper
{
     ...
     */
    private function get_package_info($package_id, $partner_id = 0, $is_admin = 0)
    {
//        $new_data = array();
        if ($partner_id) {
            // $coefficient = model('partner')->where('id', $partner_id)->value('coefficient');
        }
        $action_info = model('package_scene')->where(['package_id' => $package_id])->order('sort asc')->select();
        foreach ($action_info as $k => $v) {
            if ($is_admin == 1) {
                $v['scene_pic']?$action_info[$k]['scene_pic'] = $v['scene_pic']:false;
            }  else {
                $v['scene_pic']?$action_info[$k]['scene_pic'] = picture_url_dispose($v['scene_pic']):false;
            }
            $package_goods = model('package_goods')->where(['package_id' => $package_id, 'key' => $v['sort']])->select();
            $action_info[$k]['original_price'] = 0;
            foreach ($package_goods as $k1 => $v1) {
                ...
                if ($goods) {
                    ...
                    if ($v1['sku_id']) {
                        ...
                        if ($partner_id) {
                            $partner_coefficient = model('partner_goods')->where(['goods_id' => $v1['goods_id'], 'partner_id' => $partner_id])->value('coefficient');
                            if ($v1['sku_id']) {
                                if ($partner_coefficient) {
                                    $result['price'] > 0 ? $goods['price'] = $result['price'] * $partner_coefficient : false;
                                } 
                            }
                        }
                    } else {
                        ...
                        if ($partner_id) {
                            $partner_coefficient = model('partner_goods')->where(['goods_id' => $v1['goods_id'], 'partner_id' => $partner_id])->value('coefficient');
                            $coefficient_price = model('partner_goods')->where(['goods_id' => $v1['goods_id'], 'partner_id' => $partner_id])->value('price');
                            if ($coefficient_price > 0) {
                                $goods['price'] = $coefficient_price;
                            } else if ($partner_coefficient) {
                                $goods['price'] > 0 ? $goods['price'] = $goods['price'] * $partner_coefficient : false;
                            } 
                        }
                    }
                    $package_goods[$k1]['goods_name'] = $goods['goods_name'];
                    $package_goods[$k1]['goods_logo'] = $goods['goods_logo'];
                    $package_goods[$k1]['price'] = $goods['price'];
                    $package_goods[$k1]['spec_price'] = $goods['spec_price'];
                    $package_goods[$k1]['sku_info'] = $str;
                    $action_info[$k]['original_price'] += $goods['price'] * $v1['goods_num'];
                } else {
					unset($package_goods[$k1]);
				}
            }
			$package_goods = a_array_unique($package_goods);
            $coupon_price = 0;
            foreach ($package_goods as $k1 => $v1) {
                $package_goods[$k1]['coupon_price'] = $action_info[$k]['original_price']?round($v1['price']/$action_info[$k]['original_price']*$action_info[$k]['preferential_price'], 2):0;
                $coupon_price += $package_goods[$k1]['coupon_price']*$v1['goods_num'];
            }
            $price_dev = $action_info[$k]['preferential_price']-$coupon_price;
            if ($package_goods) {
                $package_goods[0]['coupon_price'] += $price_dev/$package_goods[0]['goods_num'];
            }
            $action_info[$k]['goods_list'] = $package_goods;
        }
        /*$action_info = json_decode($package['action_info'], true);
        foreach ($action_info as $key => $val) {
            foreach ($val as $k => $v) {
	        $cate = model('package_cate')->field('id,classname,logo_pic')->where(array('is_del' => 0, 'status' => 1, 'id' => $v['package_cate_id']))->find();
                if ($cate) {
	        $original_price = 0;
                $all_num = 0;
		$i = 0;
		if (isset($v['goods_list'])) {
                foreach ($v['goods_list'] as $k1 => $v1) {
                    $goods = model('goods')->where(array('id' => $v1['goods_id']))->find();
                    if ($goods) {
                        $sku_info = [];
                        if ($v1['sku_id']) {
                            $result = model('SpecGoodsPrice')->where(["key" => $v1['sku_id'], 'goods_id' => $v1['goods_id']])->find();
                            //$goods['spec_price'] = $result['spec_price'];
                            $goods['price'] = $result['price'];
                            if ($partner_id) {
                                $partner_coefficient = model('partner_goods')->where(['goods_id' => $v1['goods_id'], 'partner_id' => $partner_id])->value('coefficient');
                                if ($v1['sku_id']) {
                                    if ($partner_coefficient) {
                                        ...
                                        $result['price'] > 0 ? $goods['price'] = $result['price'] * $coefficient : false;
                                    }
                                }
                            }
                        } else {
                            if ($partner_id) {
                                ...
                                if ($coefficient_price > 0) {
                                    ...
                                    $goods['price'] > 0 ? $goods['price'] = $goods['price'] * $coefficient : false;
                                }
                            }
                        }


                        $v['goods_list'][$k1]['goods_name'] = $goods['goods_name'];
                        $v['goods_list'][$k1]['goods_logo'] = $goods['goods_logo'];
                        $v['goods_list'][$k1]['price'] = $goods['price'];
                        $v['goods_list'][$k1]['goods_id'] = $v1['goods_id'];
                        $v['goods_list'][$k1]['sku_id'] = $v1['sku_id'];
                        $v['goods_list'][$k1]['goods_num'] = $v1['goods_num'];
                        $v['goods_list'][$k1]['sku_info'] = $sku_info;
                        $original_price += $v1['goods_num'] * $goods['price'];
                        $all_num += $v1['goods_num'];
                    } else {
                        unset($v['goods_list'][$k1]);
                    }
                }
		
                $v['original_price'] = $original_price;
                $v['all_num'] = $all_num;
                $v['count'] = count($v['goods_list']);
                foreach ($v['goods_list'] as $kk => $vv) {
                    $v['goods_list'][$kk]['coupon_price'] =   $v['original_price']?round($vv['price']*$vv['goods_num']/$v['original_price']*$v['preferential_price']):0;
                }
                $new_data[$v['package_cate_id']][$key] = $v;
		}
		}
            }
        }
        $i = 0;
        $package['action_info'] = array();
        foreach ($new_data as $k => $v) {
            $package_cate = model('package_cate')->field('id,classname,logo_pic')->where(array('is_del' => 0, 'id' => $k))->find();
            $package['action_info'][$i]['package_cate'] = $package_cate;
            $package['action_info'][$i]['goods_info'] = $v;
            $package['action_info'][$i]['count'] = count($v);
            $i++;
        }*/
        return $action_info;
    }

    private function packageAfter($package_id, $action)
    {
        $package_scene = model('package_scene')->where(['package_id' => $package_id])->select();
        if ($package_scene) {
            model('package_scene')->where(['package_id' => $package_id])->delete();
        }
        $package_goods = model('package_goods')->where(['package_id' => $package_id])->select();
        if ($package_goods) {
            model('package_goods')->where(['package_id' => $package_id])->delete();
        }
        foreach ($action as $k => $v) {
            $data_info = [
                'package_id' => $package_id,
                'scene_name' => $v['scene_name'],
                'scene_pic' => $v['scene_pic'],
                'original_price' => $v['original_price'],
                'preferential_price' => $v['preferential_price'],
                'sort' => $k,
            ];
            model('package_scene')->isUpdate(false)->save($data_info);
            if (isset($v['goods_list'])) {
...
//                        'preferential_price' => $val['preferential_price'],
                        'sort' => $key,
                    ];
                    model('package_goods')->isUpdate(false)->save($goods_data_info);
                }
            }
        }
    }
     ...
     * @return array
     */
    private function get_goods_list_price($is_sku, $goods_id, $prom_type, $prom_id, $partner_id = 0)
    {
        $price = '';
        $spec_price = '';
        $cost_price = '';
        if ($is_sku) {
            ...
            foreach ($filter_spec as $k => $v) {
                $spec_key[] = $v['list'][0]['item_id'];
            }
            sort($spec_key);
            $spec_key = implode('_', $spec_key);
            if ($spec_key) {
                foreach ($sku_data as $k => $v) {
                    $prom = $this->get_goods_promotion($v['prom_type'], $v['prom_id'], $goods_id, $v['spec_key'], $partner_id);
                    if ($v['price']) {
                        $price = $prom['price'];
                    } else {
                        $price = $v['price'];
                ...
                $defalut_goods_info = $sku_data[$spec_key];
                if ($defalut_goods_info) {
                    $spec_price = $defalut_goods_info['spec_price'];
                    $price = $defalut_goods_info['price'];
                    $cost_price = $defalut_goods_info['cost_price'];
                }
            }
        } else {
            $prom = $this->get_goods_promotion($prom_type, $prom_id, $goods_id, 0, $partner_id);
            if ($prom['price']) {
                $spec_price = $prom['price'];
            }
        }
        return ['price' => $price, 'spec_price' => $spec_price, 'cost_price' => $cost_price];
    }
     ...
     * @throws \think\exception\DbException
     */
    private function get_spec_list($goods_id)
    {
        $keys = model('SpecGoodsPrice')->where("goods_id", $goods_id)->select();
     ...
     * @throws \think\exception\DbException
     */
    private function get_sku_des($goods_id, $spec_key)
    {
        if (!$spec_key) {
            return '';
        }
        $keys = model('SpecGoodsPrice')->where(["key" => $spec_key, 'goods_id' => $goods_id])->find();
        if (!$keys) {
            return '';
        }
        $sku_info = $keys['key_name'];
        return $sku_info;
    }
    ...
    private function get_spec($goods_id)
    {
        ...
        $keys = model('SpecGoodsPrice')->where("goods_id", $goods_id)->value("GROUP_CONCAT(`key` SEPARATOR '_') ");
        $filter_spec = [];
        if ($keys) {
            $specImage = model('SpecImage')->where(['goods_id' => $goods_id, 'src' => ['<>', '']])->field("spec_image_id,src")->select();// 规格对应的 图片表， 例如颜色
            $specImage = get_id_val($specImage, 'spec_image_id', 'src');
            $keys = str_replace('_', ',', $keys);
            $sql = "SELECT a.name,a.order,b.* FROM tb_spec AS a INNER JOIN tb_spec_item AS b ON a.id = b.spec_id WHERE b.id IN($keys) ORDER BY b.id";
            $filter_spec2 = \think\Db::query($sql);
            $filter_spec3 = [];
            foreach ($filter_spec2 as $key => $val) {
                $src = isset($specImage[$val['id']]) ? $specImage[$val['id']] : '';
                $filter_spec3[$val['name']][] = [
                    'item_id' => $val['id'],
                    'item' => $val['item'],
                    'src' => $src,
                ];
            }
            foreach ($filter_spec3 as $k => $v) {
                $filter_spec[] = [
                    'name' => $k,
                    'list' => $v,
                ];
     ...
     */
    public function footprintAdd($user_id, $goods_id)
    {
        ...
        $goodsinfo = model('goods')->where('id', $goods_id)->find();
        if (!$goodsinfo) {
            ajaxReturn(["status" => 0, "msg" => "此商品已经不存在了"]);
            return array('status' => 0, 'info' => '此商品已经不存在了');
        }

        if ($goodsinfo['is_sale'] == 0) {
            ajaxReturn(["status" => 0, "msg" => "此商品已经下架了"]);
        }

        //添加足记（当天的）
        $foot_data = [];
        $foot_data['user_id'] = $user_id;
        $foot_data['goods_id'] = $goods_id;
        $data = $foot_data;
        $now_start_time = strtotime(date('Y-m-d 00:00:00', time()));
        $now_end_time = strtotime(date('Y-m-d 23:59:59', time()));
        $data['create_time'] = ['between', [$now_start_time, $now_end_time]];
        $info = model('footprint')->where($data)->field('id')->find();
        $foot_data['update_time'] = time();
        if ($info) {
            //只是更新时间
            $res = model('footprint')->save($foot_data, ['id' => $info['id']]);
        } else {
            $foot_data['create_time'] = time();
            $res = model('footprint')->save($foot_data);
        }
        //ajaxReturn(["status" => 1, "msg"=>"添加足记成功"]);
    }
     ...
     * @return  mix
     */
    private function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
     ...
     */
    private function get_cat_tree($id)
    {
        global $goods_category, $goods_category2;
     ...
     * @return 返回数组 Description
     */
    private function remove_cat($cat_list, $parent_id_path)
    {
     ...
     * @return 返回数组 Description
     */
    private function refresh_cat($id)
    {
        $GoodsCategory = model("GoodsCategory"); // 实例化User对象
     ...
     */
    private function getAttrInput($goods_id, $type_id)
    {
     ...
     * @return array 返回数组
     */
    private function getGoodsAttrVal($goods_attr_id = 0, $goods_id = 0, $attr_id = 0)
    {
     ...
     */
    private function saveGoodsAttr($goods_id, $goods_type)
    {
        $GoodsAttr = model('GoodsAttr');
        //$Goods = model("Goods");
     ...
     * @return array 返回数组
     */
    private function getSpecItem($spec_id)
    {
     ...
     */
    private function getSpecInput($goods_id, $spec_arr, $manager_id = 0, $type = 0)
    {
        ...
        $item = model('Spec')->field('id,name')->select(); // 获取规格项
        $spec = get_id_val($item, 'id','name');// 规格表

        $item = model('SpecItem')->field('id,item,spec_id')->select();//规格项
        ...
        $item = model('SpecGoodsPrice')->where($where)->field('key,key_name,price,cost_price,store_count,bar_code,sku')->select();//规格项

        if ($manager_id) {
            $manager_cate_id = model('manager')->where(['id' => $manager_id])->value('manager_cate_id');
        } else {
            $manager_cate_id = 4;
        }
        ...
        {
            $spec[$v] = isset($spec[$v])?$spec[$v]:'';
            $str .=" <td><b>{$spec[$v]}</b></td>";
        }
        if ($is_cost_price != 1 && $type == 1) {
            $str .= "";
        } else {
            $str .= "<td><b>价格</b></td>";
        }
        ...
        foreach ($spec_arr2 as $k => $v)
        {
            ...
            isset($keySpecGoodsPrice[$item_key]) ? false : $keySpecGoodsPrice[$item_key] = [];
            isset($keySpecGoodsPrice[$item_key]['price']) ? false : $keySpecGoodsPrice[$item_key]['price'] = 0; // 价格默认为0
            isset($keySpecGoodsPrice[$item_key]['cost_price']) ? false : $keySpecGoodsPrice[$item_key]['cost_price'] = 0; // 价格默认为0
            isset($keySpecGoodsPrice[$item_key]['store_count']) ? false : $keySpecGoodsPrice[$item_key]['store_count'] = 0; //库存默认为0
            isset($keySpecGoodsPrice[$item_key]['bar_code']) ? false : $keySpecGoodsPrice[$item_key]['bar_code'] = ''; //库存默认为0
            if ($is_cost_price != 1 && $type == 1) {
                $str .="<input type='hidden' class=\"form-control\" name='item[$item_key][price]' value='{$keySpecGoodsPrice[$item_key]['price']}'>";
            } else {
            ...
            if ($is_cost_price == 1 || $type == 1) {
                $str .= "<td><input class=\"form-control\" name='item[$item_key][cost_price]' value='{$keySpecGoodsPrice[$item_key]['cost_price']}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
            } else {
                $str .= "<input type='hidden' name='item[$item_key][cost_price]' value='{$keySpecGoodsPrice[$item_key]['cost_price']}'/>";
            }
            $str .="<td><input class=\"form-control\" name='item[$item_key][store_count]' value='{$keySpecGoodsPrice[$item_key]['store_count']}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")'/></td>";
            $str .="<td><input class=\"form-control\" name='item[$item_key][bar_code]' value='{$keySpecGoodsPrice[$item_key]['bar_code']}' />
    ...
    {
        $list = model('Spec')->where("type_id = $type_id")->order('`order` desc')->select();
        //$list = model('Spec')->where("1=1")->order('`order` desc')->select();
        $str = '';
    ...
    {
        $list = model('GoodsAttribute')->where("type_id = $type_id and attr_index > 0 ")->order('`order` desc')->select();
        //$list = model('Spec')->where("1=1")->order('`order` desc')->select();
        $str = '';
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list = model('goods_category')->getField('id,parent_id,level');
    ...
    function getSortBrands()
    {
        ...
        $brandList = model("Brand")->cache(true)->select();
        $brandIdArr = model("Brand")->cache(true)->where("name in (select `name` from `" . C('database.prefix') . "brand` group by name having COUNT(id) > 1)")->getField('id,cat_id');
        $goodsCategoryArr = model('goodsCategory')->cache(true)->where("level = 1")->getField('id,name');
        $nameList = array();
    ...
    function getSortCategory()
    {
        ...
        $categoryList = model("GoodsCategory")->cache(true)->getField('id,name,parent_id,level');
        $nameList = array();
    ...
    function savePreSell($data)
    {
        ...
        if (array_key_exists('shut_down', $data) && $data['shut_down'] == 1) {
            if (!empty($data['id'])) {
                $pre_sell_shut_down = model('goods_activity')->where(array('act_id' => $data['id']))->save(array('end_time' => time()));
        ...
        $goods_main = model('goods')->where(array('goods_id' => $data['goods_id']))->find();
        if ($goods_main['store_count'] < $ext_info['restrict_amount']) {
            return array('msg' => '预售库存大于商品库存', 'status' => 0, 'url' => U('Promotion/pre_sell_info', array('id' => $data['id'])));
        }
        if (empty($data['id'])) {
            //添加一个活动的判断
            $is_have_where = array('goods_id' => $data['goods_id']);
            $is_have_goods = model('goods_activity')->where($is_have_where)->find();
            if ($is_have_goods) {
        ...
        if (!empty($data['id']) && !empty($data['goods_id'])) {
            $goods_activity = model('goods_activity')->where(array('act_id' => $data['id']))->find();
        ...
        if (empty($data['id'])) {
            ...
            model('goods')->where(array('goods_id' => $data['goods_id']))->save($goods_save_data);
            $r = model('goods_activity')->add($add_data);
            ...
            $goods_save_data['prom_type'] = 4;//预售商品类型
            model('goods')->where(array('goods_id' => $data['goods_id']))->save($goods_save_data);
            $r = model('goods_activity')->where(array('act_id' => $data['id'], 'act_type' => 1))->save($save_data);
            if ($r !== false) {
